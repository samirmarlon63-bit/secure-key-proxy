// Public assets served from /public. These paths work on any host
// (Vercel, Lovable, static hosting) — no CDN rewrite required.
export const RAVE_LOGO = "/media/rave-logo.png";
export const LOGIN_AVATAR = "/media/profile-avatar.jpeg";
export const PROFILE_AVATAR = "/media/profile-avatar.jpeg";
export const LOGIN_BG_VIDEO = "/media/login-bg.mp4";
export const PANEL_REESEND = "/media/panel-reesend.png";

export const EXAMPLE_VIDEO = "/media/rave-demo.mp4";
export const RAVE_MASCOT = "/media/rave-mascot.png";
export const RAVE_MASCOT_RED = "/media/rave-mascot-red.png";
export const RAVE_MODULES_BANNER = "/media/rave-modules-banner.png";
export const PROFILE_LOOP_VIDEO = "/media/profile-loop.mp4";
